...
def tts(text, a, hierspeech):
    ...
    with torch.no_grad():
        w2v_x, pitch = text2w2v.infer_noise_control(token, token_length, src_mel, src_length2, 
                        noise_scale=a.noise_scale_ttv, noise_scale_w=a.duratuion_temperature, 
                                                        length_scale=a.duratuion_length, denoise_ratio=a.denoise_ratio)
    ...
    **hps_t2w2v.model).cuda()
    ...
    if a.output_sr == 48000:
        audiosr = AudioSR48(h_sr48.data.n_mel_channels,
            ...
            **h_sr48.model).cuda()
        utils_tts.load_checkpoint(a.ckpt_sr48, audiosr, None)
        audiosr.eval()
        ...
        **h_sr.model).cuda()
        utils_tts.load_checkpoint(a.ckpt_sr, audiosr, None)
        audiosr.eval()
...
def main(a):
    ...
    hps = utils_tts.get_hparams_from_file(os.path.join(os.path.split(a.ckpt)[0], 'config.json'))
    hps_t2w2v = utils_tts.get_hparams_from_file(os.path.join(os.path.split(a.ckpt_text2w2v)[0], 'config.json'))
    h_sr = utils_tts.get_hparams_from_file(os.path.join(os.path.split(a.ckpt_sr)[0], 'config.json') )
    h_sr48 = utils_tts.get_hparams_from_file(os.path.join(os.path.split(a.ckpt_sr48)[0], 'config.json') )
    hps_denoiser = utils_tts.get_hparams_from_file(os.path.join(os.path.split(a.denoiser_ckpt)[0], 'config.json'))
